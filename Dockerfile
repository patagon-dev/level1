...
ENV redis_host redis-sharetribe-master.level1
ENV APP_MODE web
ENV MAINTENANCE_MODE false
ARG DB_DATABASE
ENV ENV_DB_DATABASE=$DB_DATABASE
...
RUN apt-get install -y nginx \
  ...
  && apt-get install -y mysql-client vim memcached sphinxsearch libmysqlclient-dev screen htop memcached
