  ...
  def settings_links_for(person, community=nil, restrict_for_admin=false)
    ...
    if !restrict_for_admin && (paypal_ready || stripe_ready)
      if @current_user.has_admin_rights?(@current_community)
        ...
        chilean_bank_account_path = @current_user.chilean_bank_account.present? ? edit_chilean_bank_account_path(@current_user.chilean_bank_account) : new_chilean_bank_account_path
      end
      ...
      links << {
        ...
        :icon_class => icon_class("payments"),
        :path => chilean_bank_account_path,
        :name => "payments"
      }
    end
