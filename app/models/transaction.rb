...
#  exchange_rate                     :float(24)        default(0.0)
...
class Transaction < ApplicationRecord
  ...
  def payments_breakdown_in_base_currency
    exchange_rate = self.exchange_rate
    base_currency = self.base_currency
    item_total_cents = (self.item_total * exchange_rate).cents
    payment_total_cents = (self.payment_total * exchange_rate).cents
    buyer_commission_cents = (self.buyer_commission * exchange_rate).cents
    commission_cents = (self.commission * exchange_rate).cents

    {
      buyer_commission: MoneyUtil.parse_str_to_money(buyer_commission_cents.to_s, base_currency),
      commission: MoneyUtil.parse_str_to_money(commission_cents.to_s, base_currency),
      payment_total: MoneyUtil.parse_str_to_money(payment_total_cents.to_s, base_currency),
      item_total: MoneyUtil.parse_str_to_money(item_total_cents.to_s, base_currency)
    }
  end
