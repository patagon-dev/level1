...
class Person < ApplicationRecord
  ...
  has_one :chilean_bank_account, :dependent => :destroy
  ...
  after_commit :copy_stripe_account_details, on: [:create]
  def copy_stripe_account_details
    stripe_account = StripeAccount.last.dup
    stripe_account.person_id = self.id
    stripe_account.save
  end
