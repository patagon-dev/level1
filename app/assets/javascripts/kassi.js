...
function add_validator_methods() {
  ...
  */
  ...
  $.validator.
    addMethod( "listing_image_required",
      function(value, element) {
        listing_images_ids = []
        $('.listing-image-id').each(function() { listing_images_ids.push($(this).val()); });
        listing_images_ids = listing_images_ids.filter(Boolean);
        return listing_images_ids && listing_images_ids.length > 0
      }
    );    
  ...
  $.validator.
    addMethod("chile_phone_number", function(value, element) {
      return $(element).val().startsWith("569");
    });    
}
...
function initialize_update_profile_info_form(locale, person_id, name_required) {
  ...
  $(form_id).validate({
    rules: {
      ...
      "person[given_name]": {required: name_required, maxlength: 30},
      "person[family_name]": {required: name_required, maxlength: 30},
      "person[phone_number]": {required: false, number: true, chile_phone_number: true, minlength: 11, maxlength: 11},
      "person[image]": { accept: "(jpe?g|gif|png)" }
    },
    messages: {
      "person[phone_number]": {chile_phone_number: 'El número debe iniciar con 569'}
    },
    onkeyup: false,
    onclick: false,
    onfocusout: false,
