module TransactionService
  class Order
    ...
    def price_break_down_locals
      {
         ...
         price_breakdown_in_base_currency: price_breakdown_in_base_currency
      }
    end
    ...
    def price_breakdown_in_base_currency
      usd_rate = base_exchange_rate
      order_total_cents = (self.order_total * usd_rate).cents
      item_total_cents = (self.item_total * usd_rate).cents

      {
        payment_total: MoneyUtil.parse_str_to_money(order_total_cents.to_s, Community::BASE_CURRENCY),
        order_total: MoneyUtil.parse_str_to_money(order_total_cents.to_s, Community::BASE_CURRENCY),
        item_total: MoneyUtil.parse_str_to_money(item_total_cents.to_s, Community::BASE_CURRENCY)
      }
    end
    ...
    def base_exchange_rate
      return @base_exchange_rate if defined?(@base_exchange_rate)

      converter = CurrencyConverter.new
      response = converter.exchange_rate

      @base_exchange_rate = response['usd']
    end
  end
end
