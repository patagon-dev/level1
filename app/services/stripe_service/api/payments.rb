module StripeService::API
  class Payments
    class << self
      ...
      def order_total_in_base_currency(tx)
        tx.payments_breakdown_in_base_currency
      end
      ...
      def do_create_preauth_payment(tx, gateway_fields, seller_account)
        ...
        base_price_breakdown = order_total_in_base_currency(tx)
        ...
        subtotal   = base_price_breakdown[:payment_total]
        total      = subtotal
        commission = base_price_breakdown[:commission]
        buyer_commission = base_price_breakdown[:buyer_commission]
        fee        = Money.new(0, subtotal.currency)
